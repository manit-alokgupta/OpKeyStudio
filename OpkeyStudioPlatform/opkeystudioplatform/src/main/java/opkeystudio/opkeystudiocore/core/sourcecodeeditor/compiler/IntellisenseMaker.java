...
public class IntellisenseMaker {
	public void createSenseFileOfJavaLibrary(File inputFile, File outputDir) {
		String senseFileName = inputFile.getName().split("\\.")[0] + ".sense";
		List<File> inputFiles = new ArrayList<File>();
		inputFiles.add(inputFile);
		List<ClassIntellisenseDTO> classesInfo = getAllReflectionsClassesFromJars(inputFiles);
		Utilities.getInstance().writeXMLSerializedData(
				new File(outputDir.getAbsolutePath() + File.separator + senseFileName), classesInfo);
	}

	@SuppressWarnings("rawtypes")
	private List<ClassIntellisenseDTO> getAllReflectionsClassesFromJars(List<File> jarFiles) {
